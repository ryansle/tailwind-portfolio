...
const Hero = (props: HeroProps) => {
  ...
  const toolkit = skills.filter(
    (entry): entry is PreparedSkill & { iconUrl: string } => Boolean(entry.iconUrl)
  );
  ...
  return (
    <div className='tracking-wider'>
        ...
        <div className='col-span-12 order-last xl:col-span-8 xl:order-first xl:pr-12 2xl:pr-16'>
          ...
          <div className='space-y-3'>
            <p className='text-2xl font-medium'>
              I&apos;m a <span className='text-emphasis'>{currentRole.title}</span> at{' '}
              <Link className='text-red-500 hover:underline underline-offset-2' href={currentRole.url}>
                {currentRole.employer}
              </Link>, building product UI and front-end systems.
            </p>
            <p className='text-lg text-gray-400 font-medium'>
              Away from the keyboard, I&apos;m a community builder at heart. Co-founder of <Link className='font-semibold text-white underline decoration-teal-400/70 underline-offset-4 transition hover:text-teal-300 hover:decoration-teal-300' href={ryanMeetup.urls.home}>Ryan Meetup</Link>, a not-for-profit organization for anyone and everyone named Ryan: it started as a bit between three of us at a Brooklyn bar in {ryanMeetup.founded}.
            </p>
            <p className='text-lg text-gray-400 font-medium'>
              After {ryanMeetup.yearsOrganizing} years and {ryanMeetup.events} events, we operate {ryanMeetup.chapters} chapters across the country and work with global brands to create meaningful connections in a post-COVID world.
            </p>
          </div>
          ...
          <div>
            ...
            <div className='text-gray-400 tracking-wide space-y-1 font-medium'>
              ...
              <p className='flex gap-2'>
                <span aria-hidden className='w-6 shrink-0'>📈</span>
                <span>Built <Link className='text-blue-400 hover:underline' href={ryanMeetup.urls.home}>Ryan Meetup</Link> from zero to 1,400 Ryans in one room, 115k+ followers, and $10k+ raised for charity.</span>
              </p>
              <p className='flex gap-2'>
                <span aria-hidden className='w-6 shrink-0'>🧭</span>
      ...
      <div>
        <div className='subtle-panel mt-8 flex flex-col items-start gap-4 px-5 py-5 sm:px-6 xl:flex-row xl:items-center'>
          <h2 className='text-xl font-medium tracking-wider'>
            Engineering toolkit
          </h2>
          <div className='hidden h-10 w-px border border-white/10 xl:block' />
          <div className='flex flex-wrap gap-3 sm:gap-4'>
            {toolkit.length === 0 && <p className='text-sm text-soft'>No toolkit highlights are listed yet. Explore the Skills page for more about how I work.</p>}
            {toolkit.map(({ skill, iconUrl }) => (
              ...
              >
                <div className='subtle-panel group relative flex h-12 w-12 items-center justify-center overflow-hidden rounded-surface border-white/10 p-2 transition-colors duration-(--duration-base) hover:border-white/25 sm:h-14 sm:w-14 sm:p-2.5'>
                  <div className='relative h-6 w-6 sm:h-7 sm:w-7'>
                    <Image
                      src={iconUrl}
                      fill
                      className='object-contain opacity-80 transition-all duration-(--duration-base) group-hover:scale-105 group-hover:opacity-100'
